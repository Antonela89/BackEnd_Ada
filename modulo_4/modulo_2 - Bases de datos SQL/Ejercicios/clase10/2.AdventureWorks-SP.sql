...
-- • Tablas: contact
-- • Campos: LastName
SELECT * FROM contact WHERE LastName LIKE '_s%';


-- 2. Mostrar el nombre concatenado con el apellido de las personas cuyo apellido tenga terminación (ez).
-- • Tablas: contact
-- • Campos: FirstName, LastName
SELECT CONCAT(FirstName, ' ', LastName) AS Nombre FROM contact WHERE LastName LIKE '%ez';
...
SELECT * 
FROM contact 
WHERE FirstName REGEXP '^c.[^defg][j-w]';
...
-- • Tablas: employee
-- • Campos: BirthDate
SELECT * FROM employee WHERE YEAR(BirthDate) BETWEEN 1977 AND 1981;

-- Operadores
-- 1. Mostrar el código, fecha de ingreso y horas de vacaciones de los empleados que ingresaron a partir del año 2010.
-- • Tablas: employee
-- • Campos: EmployeeID, HireDate, VacationHours
SELECT EmployeeID, HireDate, VacationHours FROM employee WHERE YEAR(HireDate) >= 2010;
...
SELECT Name, ProductNumber, ListPrice, 
       (ListPrice * 1.10) AS ListPriceWithTax 
FROM product 
...
-- • Tablas: salesperson
-- • Campos: TerritoryID, EmployeeID
SELECT * FROM salesperson WHERE TerritoryID IS NULL;

-- 2. Mostrar el peso de todos los artículos. Si el peso no estuviese definido, reemplazar por cero.
-- • Tablas: product
-- • Campos: Weight
SELECT Weight FROM product WHERE Weight IS NULL;
...
-- • Tablas: product
-- • Campos: ListPrice
SELECT AVG(ListPrice) AS Promedio FROM product;
...
SELECT ProductID, SUM(LineTotal) AS TotalVendido FROM salesorderdetail GROUP BY ProductID ORDER BY TotalVendido ASC;
...
SELECT ProductSubcategoryID FROM product WHERE ListPrice < 150 GROUP BY ProductSubcategoryID HAVING COUNT(*) >= 4;
...
-- • Tablas: product
-- • Campos: ProductSubcategoryID, ListPrice
SELECT ProductSubcategoryID, COUNT(*) AS Cantidad, AVG(ListPrice) AS Promedio FROM product WHERE ListPrice > 70 GROUP BY ProductSubcategoryID HAVING AVG(ListPrice) > 700;
...
SELECT sod.ProductID, p.Name, sod.UnitPrice, p.ListPrice 
FROM salesorderdetail sod
JOIN product p ON sod.ProductID = p.ProductID 
WHERE sod.UnitPrice < p.ListPrice 
ORDER BY p.Name;

-- 2. Mostrar todos los productos que tengan el mismo precio (precio de venta y precio de lista) y que tengan un color asignado (no nulo). Se deben mostrar de a pares, código y nombre de cada uno de los dos productos y el precio de ambos. Ordenar por precio de venta en forma descendente.
...
FROM product p1 
JOIN product p2 ON p1.ListPrice = p2.ListPrice AND p1.ProductID <> p2.ProductID 
...
-- • Tablas: product, productVendor, contact
-- • Campos: Name, ProductID, EmployeeID, ProductSubcategoryID
SELECT p.Name, c.FirstName, c.LastName 
FROM product p 
JOIN productVendor pv ON p.ProductID = pv.ProductID 
JOIN contact c ON pv.VendorID  = c.ContactID 
...
-- • Tablas: employee, contact, salesperson
-- • Campos: FirstName, LastName, LoginID, EmployeeID, TerritoryID
SELECT c.FirstName, c.LastName, e.LoginID 
FROM employee e 
JOIN contact c ON e.EmployeeID = c.ContactID 
JOIN salesperson s ON e.EmployeeID = s.SalesPersonID
