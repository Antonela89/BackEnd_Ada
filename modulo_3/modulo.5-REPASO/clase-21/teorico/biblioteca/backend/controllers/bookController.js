...
import { readBooks, writeBooks } from '../models/bookModel.js';
...
const BookController = () => {
    ...
    const getById = (req, res) => {
        ...
        if (!book) {
            return res.status(404).json({message: `Libro: ${req.params.id} no encontrado.`})
        }
    ...
    const deleteBook = (req, res) => {
        ...
        res.status(200).json({message: `Libro ${req.params.id} borrado con éxito.`})
    }
}
